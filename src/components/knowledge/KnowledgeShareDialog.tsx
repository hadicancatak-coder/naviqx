...
export function KnowledgeShareDialog({ open, onOpenChange, page }: KnowledgeShareDialogProps) {
  const queryClient = useQueryClient();
  const [copied, setCopied] = useState(false);

  // Fetch existing unified access link
  const { data: accessLink, isLoading } = useQuery({
    queryKey: ['knowledge-access-link', page.id],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('public_access_links')
        .select('*')
        .eq('resource_type', 'knowledge')
        .eq('resource_id', page.id)
        .eq('is_active', true)
        .maybeSingle();
  ...
  const toggleAccess = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['knowledge-access-link', page.id] });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Share "{page.title}" with external viewers via a public link.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-md py-md">
          ...
          <p className="text-metadata text-muted-foreground text-center">
            External viewers can read the page content. No sign-in required.
          </p>
        </div>
      </DialogContent>
    </Dialog>
